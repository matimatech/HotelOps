...
check_in_date = f"{check_in_date.day}-{check_in_date.month}-{check_in_date.year}"
print(check_in_date)
check_out_date = today + timedelta(days=8)
check_out_date = f"{check_out_date.day}-{check_out_date.month}-{check_out_date.year}"
...
def scroll_and_navigate_all_results_traveloka(page) -> list:
    """
    Scroll untuk merender seluruh konten traveloka
    ...
    while True:
        ...
        page.wait_for_selector('[id="page-content"]', timeout=10000)
        ...
        hotel_items = page.query_selector_all("//*[@data-testid='infinite-list-container']//div[@data-testid='list-card-inview-wrapper']")
        print(hotel_items)
...
def scrape_hotel(url, search_query, check_in_date, check_out_date):
    try:
        with sync_playwright() as p:
            # ── Browser args untuk menonaktifkan tanda-tanda otomasi ──────────
            ...
            # ── Buat context dengan opsi yang menyerupai pengguna nyata ───────
            ctx_opts = get_stealth_context_options()
            webgl_vendor   = ctx_opts.pop("_webgl_vendor")
            webgl_renderer = ctx_opts.pop("_webgl_renderer")

            context = browser.new_context(**ctx_opts)

            # Terapkan stealth script (webdriver, plugins, chrome obj, dll.)
            apply_stealth_mode(context)

            # Buat halaman & inject WebGL spoof lewat page.add_init_script
            # (dijalankan sebelum script halaman manapun)
            page = context.new_page()
            page.add_init_script(f"""
                window.__tvk_webgl_vendor__   = "{webgl_vendor}";
                window.__tvk_webgl_renderer__ = "{webgl_renderer}";
            """)

            # playwright_stealth sebagai lapisan tambahan
            ...
            # Simulasi perilaku manusia: gerakkan mouse & scroll perlahan
            ...
            # ── Tutup popup "Lihat sebagai tamu" jika muncul ─────────────────
            try:
                guest_btn = page.locator('//div[contains(text(),"Lihat sebagai tamu")]')
                guest_btn.wait_for(timeout=7000)
                simulate_human_mouse(page)
                guest_btn.click()
                random_delay(1, 2)
            except Exception:
            ...
            # ── Klik tab Hotel ────────────────────────────────────────────────
            page.locator('[data-testid="product-pill-Hotel"]').click()
            random_delay(0.8, 1.5)

            # ── Isi kolom pencarian dengan human_type ─────────────────────────
            search = page.locator('[data-testid="autocomplete-field"]')
            search.click()
            random_delay(0.5, 1)
            # Ketik karakter per karakter agar tidak terdeteksi bot
            ...
            simulate_human_mouse(page)
            page.locator('[data-testid="accom_autocomplete_item_0"]').click()
            random_delay(0.8, 1.5)

            # ── Pilih tanggal check-in ────────────────────────────────────────
            page.locator('[data-testid="check-in-date-field"]').click()
            random_delay(0.8, 1.5)
            page.locator(f'[data-testid="date-cell-{check_in_date}"]').click()
            random_delay(0.8, 1.5)

            # ── Pilih tanggal check-out ───────────────────────────────────────
            page.locator(f'[data-testid="date-cell-{check_out_date}"]').click()
            page.eval_on_selector(
                ".css-1dbjc4n.r-14lw9ot.r-kdyh1x.r-rs99b7.r-1d2f490.r-1udh08x.r-u8s1d.r-j76wpu.r-pezta",
                "el => el.style.display = 'none'"
            )
            random_delay(0.8, 1.5)
            print(f"check in {check_in_date}, check out {check_out_date}")

            # Gerakkan mouse sebelum submit
            simulate_human_mouse(page)
            random_delay(0.5, 1)
            page.click('[data-testid="search-submit-button"]')
            print("Search submitted")

            scraped_data = scroll_and_navigate_all_results_traveloka(page)
            scraped_data = pd.DataFrame(scraped_data)

    except Exception as e:
        print(f"Error during Traveloka scraping: {e}")

# Jalankan script
if __name__ == "__main__":
    LOCATIONS = ["Bali", "Yogyakarta", "Jakarta"]
    for location in LOCATIONS:
        scrape_hotel("https://www.traveloka.com/id-id/", location, check_in_date, check_out_date)
